...
if "input_name" not in st.session_state:
    st.session_state.update({"input_name":"", "input_ingredients":"", "input_steps":"", "input_notes":"", "input_improvement":"", "input_videos":[""]*5, "edit_index":None, "active_tab":0})
...
if st.session_state["active_tab"] == 0:
    ...
    with st.form("recipe_form"):
        ...
        for i in range(5):
            with cols_vid[i]:
                new_vids.append(st.text_input(f"影片 {i+1}", value=st.session_state["input_videos"][i]))
        ...
        if st.form_submit_button("✨ 一鍵自動排版"):
            st.session_state.update({"input_name": recipe_name, "input_videos": new_vids, 
    ...
    for index, row in df.iterrows():
        with st.expander(f"🍞 {row['name']}"):
            ...
            if st.button("✏️ 帶入編輯", key=f"edit_{index}"):
                st.session_state.update({"input_name": row['name'], "input_ingredients": row['ingredients'], "input_steps": row['steps'], "input_notes": row['notes'], "input_videos": (vids + [""]*5)[:5], "edit_index": index, "active_tab": 0})
                st.rerun()
