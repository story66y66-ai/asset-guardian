...
with col_btn1:
  ...
  st.markdown(
      f'<a href="{yt_url_top}" target="_blank" class="yt-button">🔥'
      " 太極教學熱門影片任意門</a>",
      unsafe_allow_html=True,
  )
with col_btn2:
...
if "taiji_playlist_names" not in st.session_state:
  st.session_state.taiji_playlist_names = {
...
def save_taiji_data_to_csv():
  ...
  for idx in range(1, max_idx + 1):
    csv_data_list.append({
        ...
        "url": st.session_state.get(
            f"taiji_yt_input_url_{idx}", ""
        ).strip(),
        "title": st.session_state.taiji_playlist_names.get(
            idx, f"套路 {idx}"
        ),
  ...
  df_export.to_csv(
      TAIJI_CSV_FILE, index=False, encoding="utf-8-sig", quoting=1
  )
...
with st.expander(
    "📥 產生並下載包含完整太極招式的 CSV 庫（支援 Excel 直接開啟與 GitHub）"
):
  st.markdown(
      "<span style='font-size: 20px; font-weight: bold;'>系統已啟用自動儲存與聯動帶入，點擊選項即可無縫切換！</span>",
      unsafe_allow_html=True,
  )

  st.download_button(
      label=f"📥 點我直接下載 {TAIJI_CSV_FILE} 檔案",
...
st.markdown(
    "<h3 style='font-size: 26px; color: #ffffff;'>📚 選擇太極練習冊（翻書頁面）：</h3>",
    unsafe_allow_html=True,
)
...
for p in range(1, 7):
  with page_cols[p - 1]:
    btn_label = (
        ...
        if st.session_state.taiji_current_page == p
        else f"第 {p} 頁\n{st.session_state.taiji_page_names[p]}"
    )
    if st.button(
        btn_label, key=f"taiji_page_btn_{p}", use_container_width=True
    ):
...
with st.expander(f"✏️ 自訂【第 {current_page} 頁】的用途與名稱設定"):
  ...
  with col_p1:
    new_page_name = st.text_input(
        f"第 {current_page} 頁總用途名稱：",
        value=st.session_state.taiji_page_names[current_page],
        key=f"input_taiji_page_name_{current_page}",
    )
  ...
  with tab:
    ...
    with st.expander(f"✏️ 修改【單元 {absolute_idx}】的套路名稱"):
      ...
      new_track_name = st.text_input(
          ...
          value=curr_name,
          key=f"rename_taiji_track_{absolute_idx}",
      )
    ...
    with left_col:
      ...
      with col_copy1:
        copy_btn = st.button("📋 複製網址", key=f"copy_taiji_{absolute_idx}")
      with col_copy2:
        if copy_btn:
          if user_yt_link.strip():
    ...
    with right_col:
      ...
      with btn_col:
        ...
        st.markdown(
            f'<a href="{translate_url}" target="_blank"'
            ' class="translate-button">🌐 Google 搜尋</a>',
            unsafe_allow_html=True,
        )
      ...
      with col1:
        play_btn = st.button("🔊 播放整段", key=f"play_taiji_{absolute_idx}")
      with col2:
        smart_split_btn = st.button(
            "✨ 自動換行排版", key=f"split_taiji_{absolute_idx}"
        )
      with col3:
        clear_btn = st.button("🗑️ 清空", key=f"clear_taiji_{absolute_idx}")
      ...
      if smart_split_btn and user_input_text.strip():
        lines = [
            l.strip() for l in user_input_text.split("\n") if l.strip()
        ]
        formatted_result = "\n".join(lines)
    ...
    if taiji_lines:
      st.markdown(
          f"<span style='font-size: 20px;'>**已自動抓取 {len(taiji_lines)}"
          " 個招式進行逐句練習：**</span>",
          unsafe_allow_html=True,
      )
      ...
      for line_idx, taiji_sentence in enumerate(taiji_lines):
        st.markdown("---")
        st.markdown(
            f"<div class='sentence-display'>第 {line_idx + 1}"
            f" 招原招式：<br>🥋 {taiji_sentence}</div>",
            unsafe_allow_html=True,
        )
        ...
        with cols[0]:
          if st.button(
              "🔊 聽發音",
              key=f"taiji_line_audio_{absolute_idx}_{line_idx}",
          ):
            try:
        ...
        with cols[3]:
          st.button(
              "🗑️ 清除",
              key=f"clear_taiji_line_{absolute_idx}_{line_idx}",
              on_click=make_clear_callback(ans_key),
          )

        if user_answer.strip():
          if user_answer.strip() == taiji_sentence:
            st.markdown(
                "<span style='font-size: 22px; color: #28a745; font-weight:"
                " bold;'>🎉 答對了！招式名稱正確，太棒囉！</span>",
                unsafe_allow_html=True,
            )
          else:
            st.markdown(
                "<span style='font-size: 22px; color: #ff4b4b; font-weight:"
                " bold;'>❌ 名稱有出入，再試一次看看喔！<br>💡"
                f" 正確解答提示：{taiji_sentence}</span>",
                unsafe_allow_html=True,
            )
    else:
      st.info(
          "💡"
          " 請在上方右側的文字框輸入太極招式（一行一招），下方就會自動產生對應的逐招練習題囉！"
      )
